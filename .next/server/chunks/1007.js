...
`}];var h=c(98220);let i=(0,f.join)(process.cwd(),"data","traininggeeks.db"),j=globalThis;function k(){if(!j.__tgDb){let a=function(a=i){if(":memory:"!==a){let b=(0,f.dirname)(a);(0,e.existsSync)(b)||(0,e.mkdirSync)(b,{recursive:!0})}let b=new d.DatabaseSync(a);b.exec("PRAGMA journal_mode = WAL;"),b.exec("PRAGMA foreign_keys = ON;");b.exec(`CREATE TABLE IF NOT EXISTS _migrations (
    ...
    applied_at TEXT NOT NULL DEFAULT (datetime('now'))
     ...
     ORDER BY valid_from DESC LIMIT 1`,b,c,d);return f?.value}function i(a,b,c,d,e){a.prepare("INSERT INTO threshold (curve, metric, value, valid_from) VALUES (?, ?, ?, ?)").run(b,c,d,e)}function j(a,b,c,d,e){a.prepare("DELETE FROM zone WHERE curve = ? AND metric = ?").run(b,c);let f=a.prepare("INSERT INTO zone (curve, metric, zone_index, name, low, high, valid_from) VALUES (?, ?, ?, ?, ?, ?, ?)");d.forEach((a,d)=>f.run(b,c,d,a.name,a.low,a.high,e))}c.d(b,{$x:()=>Z,$y:()=>p,Bj:()=>z,Bt:()=>Q,C4:()=>C,DZ:()=>N,F9:()=>M,G9:()=>S,I1:()=>P,J6:()=>D,KX:()=>i,Lg:()=>h,NW:()=>F,Ox:()=>L,PH:()=>O,PL:()=>T,Pj:()=>w,Q$:()=>u,R1:()=>y,SX:()=>W,T1:()=>G,Ul:()=>V,VP:()=>R,Wg:()=>Y,Xo:()=>r,YC:()=>x,YR:()=>n,ZC:()=>U,aN:()=>j,aZ:()=>m,bQ:()=>q,b_:()=>J,c9:()=>B,d1:()=>I,eF:()=>o,jH:()=>l,k8:()=>s,lz:()=>X,pL:()=>v,pt:()=>g,sQ:()=>A,uH:()=>E,vt:()=>K,wR:()=>H,wc:()=>f,zg:()=>t});let k=["modality","sport_detail","source","fit_hash","raw_path","start_time","local_date","timezone","name","notes","private_notes","duration_s","elapsed_s","distance_m","elevation_gain_m","avg_hr","max_hr","avg_power","max_power","np","avg_speed_mps","max_speed_mps","avg_cadence","max_cadence","calories","kj","tss","s3","intensity_factor","variability_index","efficiency_factor","decoupling","rpe","route_polyline","metrics_version"];function l(a,b){let c=k.filter(a=>void 0!==b[a]),d=c.map(()=>"?").join(", "),e=c.map(a=>b[a]??null);return Number(a.prepare(`INSERT INTO activity (${c.join(", ")}) VALUES (${d})`).run(...e).lastInsertRowid)}function m(a,b){return e(a,"SELECT * FROM activity WHERE id = ?",b)}function n(a,b,c){return d(a,`SELECT * FROM activity WHERE local_date >= ? AND local_date <= ?
     ORDER BY start_time`,b,c)}function o(a,b,c){let d=Object.keys(c).filter(a=>"id"!==a&&"created_at"!==a);if(0===d.length)return;let e=d.map(a=>`${a} = ?`).join(", "),f=d.map(a=>c[a]);a.prepare(`UPDATE activity SET ${e}, updated_at = datetime('now') WHERE id = ?`).run(...f,b)}function p(a,b){a.prepare("DELETE FROM activity WHERE id = ?").run(b)}function q(a,b){return d(a,"SELECT * FROM activity_file WHERE activity_id = ? ORDER BY created_at, id",b)}function r(a,b,c){a.prepare("DELETE FROM strength_set WHERE activity_id = ?").run(b);let d=a.prepare(`INSERT INTO strength_set (activity_id, set_index, exercise_key, exercise_name,
       reps, duration_s, rest_s, weight_kg)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);c.forEach((a,c)=>d.run(b,a.set_index??c,a.exercise_key,a.exercise_name??null,a.reps,a.duration_s,a.rest_s,a.weight_kg))}function s(a,b){let c=e(a,"SELECT channels FROM activity_stream WHERE activity_id = ?",b);return c?JSON.parse(c.channels):void 0}function t(a,b){a.prepare(`INSERT INTO daily_load (date, curve, stress, ctl, atl, tsb)
     VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT (date, curve) DO UPDATE SET
       stress = excluded.stress, ctl = excluded.ctl,
       atl = excluded.atl, tsb = excluded.tsb`).run(b.date,b.curve,b.stress,b.ctl,b.atl,b.tsb)}function u(a){return d(a,"SELECT local_date, modality, tss, s3 FROM activity")}function v(a){a.prepare("DELETE FROM daily_load").run()}function w(a){let b=e(a,"SELECT MIN(local_date) AS d FROM activity");return b?.d??void 0}function x(a,b){return Number(a.prepare(`INSERT INTO planned_workout
         (modality, date, name, description, planned_duration_s, planned_distance_m, planned_tss, source, structure, template_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(b.modality,b.date,b.name??null,b.description??null,b.planned_duration_s??null,b.planned_distance_m??null,b.planned_tss??null,b.source,b.structure??null,b.template_id??null).lastInsertRowid)}function y(a,b,c){a.prepare("UPDATE planned_workout SET date = ?, updated_at = datetime('now') WHERE id = ?").run(c,b)}function z(a,b){return e(a,"SELECT * FROM planned_workout WHERE id = ?",b)}function A(a,b,c){let d=Object.keys(c);if(0===d.length)return;let e=d.map(a=>`${a} = ?`).join(", "),f=d.map(a=>c[a]);a.prepare(`UPDATE planned_workout SET ${e}, updated_at = datetime('now') WHERE id = ?`).run(...f,b)}function B(a,b,c){a.prepare("UPDATE planned_workout SET completed_activity_id = ?, updated_at = datetime('now') WHERE id = ?").run(c,b)}function C(a,b){a.prepare("DELETE FROM planned_workout WHERE id = ?").run(b)}function D(a,b){a.prepare(`DELETE FROM planned_workout
     WHERE source = ? AND completed_activity_id IS NULL`).run(b)}function E(a,b,c){a.prepare("UPDATE activity SET local_date = ?, updated_at = datetime('now') WHERE id = ?").run(c,b)}function F(a,b){return Number(a.prepare(`INSERT INTO workout_template
         (name, modality, description, steps, est_duration_s, est_distance_m, est_tss)
       VALUES (?, ?, ?, ?, ?, ?, ?)`).run(b.name,b.modality,b.description??null,b.steps,b.est_duration_s??null,b.est_distance_m??null,b.est_tss??null).lastInsertRowid)}function G(a,b,c){a.prepare(`UPDATE workout_template
        SET name = ?, modality = ?, description = ?, steps = ?,
            est_duration_s = ?, est_distance_m = ?, est_tss = ?,
            updated_at = datetime('now')
      WHERE id = ?`).run(c.name,c.modality,c.description??null,c.steps,c.est_duration_s??null,c.est_distance_m??null,c.est_tss??null,b)}function H(a,b){return e(a,"SELECT * FROM workout_template WHERE id = ?",b)}function I(a){return d(a,"SELECT * FROM workout_template ORDER BY updated_at DESC, id DESC")}function J(a,b){a.prepare("DELETE FROM workout_template WHERE id = ?").run(b)}function K(a,b,c){a.prepare(`INSERT INTO connector_account (provider, api_key, athlete_id, last_sync_cursor, enabled)
     VALUES (?, ?, ?, ?, ?)
     ON CONFLICT (provider) DO UPDATE SET
       ...
       updated_at = datetime('now')`).run(b,c.api_key??null,c.athlete_id??null,c.last_sync_cursor??null,c.enabled??0)}function L(a,b,c,d,e){a.prepare(`INSERT INTO metric (type, date, value, notes) VALUES (?, ?, ?, ?)
     ON CONFLICT (type, date) DO UPDATE SET value = excluded.value, notes = excluded.notes`).run(b,c,d,e??null)}function M(a,b){return Number(a.prepare("INSERT INTO injury (title, body_part, start_date, notes) VALUES (?, ?, ?, ?)").run(b.title,b.body_part??null,b.start_date,b.notes??null).lastInsertRowid)}function N(a,b,c){a.prepare("UPDATE injury SET end_date = ? WHERE id = ?").run(c,b)}function O(a,b){a.prepare("DELETE FROM injury WHERE id = ?").run(b)}function P(a,b){return Number(a.prepare("INSERT INTO equipment (type, name, brand, distance_m) VALUES (?, ?, ?, ?)").run(b.type,b.name,b.brand??null,b.distance_m??0).lastInsertRowid)}function Q(a,b,c){a.prepare("UPDATE equipment SET distance_m = distance_m + ? WHERE id = ?").run(c,b)}function R(a,b,c){a.prepare("UPDATE equipment SET active = ? WHERE id = ?").run(+!!c,b)}function S(a,b){a.prepare("DELETE FROM equipment WHERE id = ?").run(b)}function T(a,b){let c=e(a,"SELECT value FROM app_setting WHERE key = ?",b);return c?.value??null}function U(a,b,c){a.prepare(`INSERT INTO app_setting (key, value) VALUES (?, ?)
     ON CONFLICT (key) DO UPDATE SET value = excluded.value, updated_at = datetime('now')`).run(b,c)}function V(a,b,c,d){a.prepare("INSERT INTO event (name, date, notes) VALUES (?, ?, ?)").run(b,c,d??null)}function W(a,b){a.prepare("DELETE FROM event WHERE id = ?").run(b)}function X(a,b){a.prepare("INSERT INTO goal (text) VALUES (?)").run(b)}function Y(a,b,c){a.prepare("UPDATE goal SET done = ? WHERE id = ?").run(+!!c,b)}function Z(a,b){a.prepare("DELETE FROM goal WHERE id = ?").run(b)}}};